...
class Sale_serviceResource(resources.ModelResource):
    class Meta:
        model = Sale_service
        fields = ('id', 'cite','service', 'unit_price', 'sale_date', 'client_name', 'client_email', 'client_id', 'garantia', )

@admin.register(Sale_service)
class Sale_serviceAdmin(ImportExportModelAdmin):
    form = SaleServiceForm
    list_display = ('id','cite', 'service', 'total_price', 'sale_date', 'display_garantia',)  # Ajusta esto si el campo no se llama 'cite'
